...
static void *gtkwavetcl_strToPointer(Tcl_Obj *const obj)
{
#if __WORDSIZE == 64
	return (void *)strtoull(Tcl_GetString(obj), 0, 0);
#else
	return (void *)strtoul(Tcl_GetString(obj), 0, 0);
#endif
}
...
static int gtkwavetcl_getTreeNodeChild(ClientData clientData, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[])
{
	char tmp[16];
	if (objc == 1) {
		// Return root
		sprintf(tmp, "0x%p", GLOBALS->treeroot);
		Tcl_SetObjResult(interp, Tcl_NewStringObj(tmp, -1));
	} else if (objc == 2) {
		struct tree *tree_n = (struct tree *)gtkwavetcl_strToPointer(objv[1]);

		sprintf(tmp, "0x%p", tree_n->child);
		Tcl_SetObjResult(interp, Tcl_NewStringObj(tmp, -1));
	} else {
		return TCL_ERROR;
	}
...
// Describe the node as:
// - id
// - kind
// - name
// - has_children
static Tcl_Obj *gtkwavetcl_describeNode(Tcl_Interp *interp, struct tree *node)
{
	char tmp[16];
	Tcl_Obj *info = Tcl_NewListObj(0, NULL);

	sprintf(tmp, "0x%p", node);
	Tcl_ListObjAppendElement(interp, info, Tcl_NewStringObj(tmp, -1));

	sprintf(tmp, "%d", node->kind);
	Tcl_ListObjAppendElement(interp, info, Tcl_NewStringObj(tmp, -1));

	Tcl_ListObjAppendElement(interp, info, Tcl_NewStringObj(node->name, -1));

	Tcl_ListObjAppendElement(interp, info,
			Tcl_NewStringObj((node->child)?"1":"0", -1));

	return info;
}

static int gtkwavetcl_getTreeNodeRoots(ClientData clientData, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[])
{
	struct tree *root = GLOBALS->treeroot;
	Tcl_Obj *root_l = Tcl_NewListObj(0, NULL);

	while (root) {
		Tcl_ListObjAppendElement(interp, root_l,
				gtkwavetcl_describeNode(interp, root));

		root = root->next;
	}
	Tcl_SetObjResult(interp, root_l);

	return (TCL_OK);
}

static int gtkwavetcl_getTreeNodeChildren(ClientData clientData, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[])
{
	char tmp[16];
	if (objc == 2) {
		Tcl_Obj *child_l = Tcl_NewListObj(0, NULL);
		struct tree *tree_n = (struct tree *)gtkwavetcl_strToPointer(objv[1]);

		if (tree_n->child) {
			tree_n = tree_n->child;
			while (tree_n) {
				Tcl_ListObjAppendElement(interp, child_l,
						gtkwavetcl_describeNode(interp, tree_n));
				tree_n = tree_n->next;
			}
		}

		Tcl_SetObjResult(interp, child_l);
	} else {
		return TCL_ERROR;
	}
...
static int gtkwavetcl_getTreeNodeFullName(ClientData clientData, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[])
{
	if (objc == 2) {
		struct tree *tree_n = (struct tree *)gtkwavetcl_strToPointer(objv[1]);

		if (tree_n->t_which >= 0) {
			Tcl_SetObjResult(interp, Tcl_NewStringObj(GLOBALS->facs[tree_n->t_which]->name, -1));
		} else {
			Tcl_SetObjResult(interp, Tcl_NewStringObj(tree_n->name, -1));
		}
	} else {
		return TCL_ERROR;
	}
...
static int gtkwavetcl_getTreeNodeKind(ClientData clientData, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[])
{
	if (objc == 2) {
		struct tree *tree_n = (struct tree *)gtkwavetcl_strToPointer(objv[1]);
		const char *kind = 0;
		...
		switch (tree_n->kind) {
			case TREE_UNKNOWN: kind = "TREE_UNKNOWN"; break;
			case TREE_SIGNAL: kind = "SIGNAL"; break;
			case TREE_IN: kind = "IN"; break;
			case TREE_OUT: kind = "OUT"; break;
			case TREE_INOUT: kind = "INOUT"; break;
			case TREE_VECTOREL: kind = "VECTOREL"; break;
			case TREE_RECORDEL: kind = "RECORDEL"; break;
			case TREE_INSTANCE: kind = "INSTANCE"; break;
			case TREE_PACKAGE: kind = "PACKAGE"; break;
			case TREE_BASE: kind = "BASE"; break;
			case TREE_VCD_ST_MODULE: kind = "MODULE"; break;
			case TREE_VCD_ST_TASK: kind = "TASK"; break;
			case TREE_VCD_ST_FUNCTION: kind = "FUNCTION"; break;
			case TREE_VCD_ST_BEGIN: kind = "BEGIN"; break;
			case TREE_VCD_ST_FORK: kind = "FORK"; break;
			case TREE_VCD_ST_GENERATE: kind = "GENERATE"; break;
			case TREE_VCD_ST_STRUCT: kind = "STRUCT"; break;
			case TREE_VCD_ST_UNION: kind = "UNION"; break;
			case TREE_VCD_ST_CLASS: kind = "CLASS"; break;
			case TREE_VCD_ST_INTERFACE: kind = "INTERFACE"; break;
			case TREE_VCD_ST_PACKAGE: kind = "PACKAGE"; break;
			case TREE_VCD_ST_PROGRAM: kind = "PROGRAM"; break;
			case TREE_VHDL_ST_DESIGN: kind = "DESIGN"; break;
			case TREE_VHDL_ST_BLOCK: kind = "BLOCK"; break;
			case TREE_VHDL_ST_GENIF: kind = "GENIF"; break;
			case TREE_VHDL_ST_GENFOR: kind = "GENFOR"; break;
			case TREE_VHDL_ST_INSTANCE: kind = "INSTANCE"; break;
			case TREE_VHDL_ST_PACKAGE: kind = "PACKAGE"; break;
		}

		if (kind) {
			Tcl_SetObjResult(interp, Tcl_NewStringObj(kind, -1));
		} else {
			char tmp[16];
			sprintf(tmp, "UNKNOWN_%d", tree_n->kind);
			Tcl_SetObjResult(interp, Tcl_NewStringObj(tmp, -1));
		}
	} else {
		return TCL_ERROR;
...
tcl_cmdstruct gtkwave_commands[] =
	...
	{"getTreeNodeChildren", gtkwavetcl_getTreeNodeChildren},
	{"getTreeNodeFullName", gtkwavetcl_getTreeNodeFullName},
	{"getTreeNodeRoots", gtkwavetcl_getTreeNodeRoots},
